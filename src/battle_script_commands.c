...
static void Cmd_adjustnormaldamage(void)
{
    ...
    if (holdEffect == HOLD_EFFECT_FOCUS_SASH && gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
    {
        RecordItemEffectBattle(gBattlerTarget, holdEffect);
        gSpecialStatuses[gBattlerTarget].focusBanded = 1;
    }
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove]) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_MOVE_PHYSICAL(gBattleMoves[gCurrentMove]) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                {
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        case STATUS1_BURN:
            ...
            break;
        case STATUS1_FREEZE:
            // add handling of primary moves once they exist
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_RAPIDSPIN:
                ...
                break;
            case MOVE_EFFECT_REMOVE_STATUS: // Smelling salts & Wake up slap
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
         ...
         && gBattleMons[gActiveBattler].hp == 0)
        {
            ...
            if (gBattleMons[gBattlerAttacker].hp != 0 && gBattleMons[gBattlerAttacker].ability == ABILITY_VICTORY_DRAW)
            {
                BattleScriptPushCursorAndCallback(BattleScript_VictoryDrawActivates);
                gBattleScripting.battler = gBattlerAttacker;
            }
...
static void Cmd_pickup(void)
{
    s32 i;
    u32 j;
    u16 species, heldItem, opponentItem;
    u32 ability;
